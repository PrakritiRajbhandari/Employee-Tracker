...
async function main() {
  ...
  let employeesList = await db.query(
    `SELECT * FROM Employee_tracker.employee;`
  );
  let employeeNameList = await db.query(
    `SELECT employee_id, CONCAT( first_name, " ", last_name ) AS fullname FROM Employee_tracker.employee;`
  );
  ...
  for (var i = 0; i < employeeNameList.length; i++) {
    ...
    newEmployeeNameList.push(
      "id: " + employeeObj.emp_id + " " + employeeObj.emp_fullname
    );
  }
  ...
  const userActivityType = await inquirer.prompt([
    {
      type: "list",
  ...
  switch (whatToDo) {
    ...
    case "Add":
      ...
      switch (whatToAdd) {
        case "Departments":
          const userResNewDept = await inquirer.prompt([
            {
              type: "input",
          ...
          await db.query(`INSERT INTO department (name) VALUES(?);`, [
            userResNewDept.departmentName,
          ]);
          showMenu();
          break;
        case "Roles":
          const userResNewRole = await inquirer.prompt([
            {
              type: "input",
          ...
          await db.query(
            ...
            [
              userResNewRole.newRoleTitle,
              userResNewRole.newRoleSalary,
              newRoleDeptId,
            ]
          );
          ...
          break;
        case "Employees":
          const userResNewEmployee = await inquirer.prompt([
            {
              type: "input",
          ...
          await db.query(
            ...
            [
              userResNewEmployee.employeeFirstName,
              userResNewEmployee.employeeLastName,
              role_id,
            ]
          );
          console.log(
    ...
    //=========new View
    case "View":
      const userResView = await inquirer.prompt([
        {
          type: "list",
      ...
      switch (whatToView) {
        ...
        case "View Employees by Department":
          const emplByDept = await inquirer.prompt([
            {
              type: "list",
          ...
          let depToView = await db.query(
            `SELECT department.name,employee.first_name,employee.last_name,role.title FROM department,role,employee WHERE department.department_id=role.department_id AND role.role_id=employee.role_id AND department.department_id = ?;`,
            [DeptVieByEmpArray[0]]
          );
        ...
        case "View Employees by Role":
          const emplByRole = await inquirer.prompt([
            {
              type: "list",
          ...
          newEmplByRole = await db.query(
            `SELECT role.title, employee.first_name, employee.last_name, role.salary FROM role,employee,department WHERE role.role_id=employee.role_id AND department.department_id=role.department_id AND role.role_id = ?;`,
            [respEmplByRoleArray[0]]
          );
          console.table(newEmplByRole);
          showMenu();
          break;
        case "view All":
          let allDatabase = await db.query(
            `SELECT * FROM employee LEFT JOIN role ON employee.role_id = role.role_id LEFT JOIN department ON role.department_id = department.department_id;`
          );
    ...
    //=========new View
    case "Update":
      const userResUpd = await inquirer.prompt([
        {
          type: "list",
    ...
    //=========new View
    case "Remove":
      const userResRemv = await inquirer.prompt([
        {
          type: "list",
      ...
      switch (sectionToRemv) {
        case "Departments":
          const userResDelDept = await inquirer.prompt([
            {
              type: "list",
          ...
          await db.query(`DELETE FROM department WHERE name = ?;`, [
            userResDelDept.departmentToDelete,
          ]);
          console.log(
            `${userResDelDept.departmentToDelete} has been deleted successfully`
          );
          showMenu();
          break;
        case "Roles":
          const userResDelRoles = await inquirer.prompt([
            {
              type: "list",
          ...
          console.log(
            `${userResDelRoles.roleToDelete} has been deleted successfully`
          );
          showMenu();
          break;
        case "Employees":
          const userResDelEmployees = await inquirer.prompt([
            {
              type: "list",
          ...
          await db.query(
            `DELETE FROM employee WHERE first_name = ? AND last_name = ? AND employee_id = ?;`,
